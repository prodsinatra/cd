...
By default the file is also made unique at the *tag* level: a random id is
embedded in the comment/software fields (existing tags are preserved unless
``--no-preserve-tags`` is given). Pass ``--no-metadata`` to change only the
audio data and leave tags untouched.

Usage:
    python uniquify_audio.py input.mp3 [output.mp3] [--seed N]
                             [--strength subtle|normal|high]
                             [--no-metadata] [--no-preserve-tags]
                             [--list-formats]
...
def uniquify_audio_file(
    ...
    When `metadata` is true (the default) the output also gets a unique id
    embedded in its tags (and, by default, the source's existing tags
    preserved), so the file is unique at the metadata level too. Pass
    `metadata=False` to change only the audio data. The same `seed`
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="do NOT embed a unique id in the tags; only change the audio "
             "data (metadata uniqueness is on by default)",
    )
    parser.add_argument(
        "--no-preserve-tags", action="store_true",
        help="when embedding metadata, drop the source's existing tags "
    ...
    try:
        uniquify_audio_file(
            ...
            metadata=not args.no_metadata, preserve_tags=not args.no_preserve_tags,
        )
